...
#include "pluginImplement.h"
#include "util/cuda/mathFunctions.h"
#include <vector>
#include <algorithm>
...
//SoftmaxPlugin::SoftmaxPlugin(int axis, const void* buffer, size_t size)
//{
//    printf("");
//}
//
...
nvinfer1::IPlugin* PluginFactory::createPlugin(const char* layerName, const nvinfer1::Weights* weights, int nbWeights)
{
    ...
    if (!strcmp(layerName, "conv4_3_norm"))
    {
        ...
        mNormalizeLayer = std::unique_ptr<INvPlugin, decltype(nvPluginDeleter)>
                (createSSDNormalizePlugin(weights, false, false, 0.001), nvPluginDeleter);//eps 0.0001
